...
func New(cfg config.Config, db *gorm.DB, redisClient *redis.Client, logger *slog.Logger) *gin.Engine {
	...
	transferManifestRepository := repository.NewTransferManifestRepository(db, wasteGeneratorRepository)
	complianceCheckRepository := repository.NewComplianceCheckRepository(db)
	wasteGeneratorService := service.NewWasteGeneratorService(wasteGeneratorRepository, securityService, transferManifestRepository)
