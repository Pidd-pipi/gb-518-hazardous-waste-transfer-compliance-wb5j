...
func New(cfg config.Config, db *gorm.DB, redisClient *redis.Client, logger *slog.Logger) *gin.Engine {
	...
	wasteGeneratorService := service.NewWasteGeneratorService(wasteGeneratorRepository, transferManifestRepository, securityService)
	...
	api.GET("/quota-usage", systemHandler.QuotaUsage)
