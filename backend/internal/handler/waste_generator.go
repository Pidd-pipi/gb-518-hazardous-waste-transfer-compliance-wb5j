...
type WasteGeneratorHandler struct{ service service.WasteGeneratorService }

func NewWasteGeneratorHandler(s service.WasteGeneratorService) *WasteGeneratorHandler {
	return &WasteGeneratorHandler{service: s}
}

func (h *WasteGeneratorHandler) Register(group *gin.RouterGroup) {
	resource := group.Group("/generators")
	resource.GET("", h.list)
...
func (h *WasteGeneratorHandler) list(c *gin.Context) {
	query := bindPage(c)
...
func (h *WasteGeneratorHandler) get(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
...
func (h *WasteGeneratorHandler) create(c *gin.Context) {
	var input dto.CreateWasteGenerator
...
func (h *WasteGeneratorHandler) update(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
	var input dto.UpdateWasteGenerator
	if err := c.ShouldBindJSON(&input); err != nil {
		util.Fail(c, http.StatusBadRequest, "invalid_request", err.Error())
		return
	}
...
func (h *WasteGeneratorHandler) transition(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
...
func (h *WasteGeneratorHandler) remove(c *gin.Context) {
