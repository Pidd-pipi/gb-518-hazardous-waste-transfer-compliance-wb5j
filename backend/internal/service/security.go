...
func newAuditLog(actor, requestID, action, entityType, before, after, detail string) *model.AuditLog {
	if actor == "" {
		actor = "system"
	}
	if requestID == "" {
	...
	return &model.AuditLog{
		Actor: actor, RequestID: requestID, Action: action, EntityType: entityType,
		BeforeState: before, AfterState: after, Detail: detail, CreatedAt: time.Now().UTC(),
	}
}
